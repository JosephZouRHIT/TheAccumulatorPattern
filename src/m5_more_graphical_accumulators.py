...
def draw_squares_from_circle(n, circle, window):
    ...
    for _ in range(n):
        ...
        point = rg.Point(square.center.x + circle.radius, square.center.y + circle.radius)
    window.render()
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    relativepoint = rg.Point((rectangle.corner_1.x + rectangle.corner_2.x) / 2, (rectangle.corner_1.y + rectangle.corner_2.y) / 2)
    radiusx = abs((rectangle.corner_2.y - rectangle.corner_1.y) / 2)
    radiusy = abs((rectangle.corner_2.x - rectangle.corner_1.x) / 2)
    x1 = min(rectangle.corner_1.x, rectangle.corner_2.x) - radiusx
    x2 = relativepoint.x
    y1 = relativepoint.y
    y2 = min(rectangle.corner_1.y, rectangle.corner_2.y) - radiusy
    centerx = rg.Point(x1, y1)
    centery = rg.Point(x2, y2)
    for _ in range(m):
        circle = rg.Circle(centerx, radiusx)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
        centerx.x -= 2 * radiusx
    for num in range(n):
        circle = rg.Circle(centery, radiusy)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    x2 = (rectangle2.corner_2.x + rectangle2.corner_1.x) / 2
    y1 = (rectangle1.corner_2.y + rectangle1.corner_1.y) / 2
    y2 = (rectangle2.corner_2.y + rectangle2.corner_1.y) / 2
    point1 = rg.Point(x1, y1)
    point2 = rg.Point(x2, y2)
    point1_1 = rg.Point(min(rectangle1.corner_1.x, rectangle1.corner_2.x), max(rectangle1.corner_1.y, rectangle1.corner_2.y))
    relativex = point1_1.x - x1
    relativey = point1_1.y - y1
    for _ in range(n):
